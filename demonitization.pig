raw_data = load '/users/miniProject/demonitization-tweets.csv' using PigStorage(',');
...
dictionary = load '/users/miniProject/AFINN.txt' using PigStorage('\t') as (word:chararray,rating:int);
...
store positive_tweets INTO '/users/miniProject/positive_tweets' using PigStorage(',');

store negative_tweets INTO '/users/miniProject/negative_tweets' using PigStorage(',');